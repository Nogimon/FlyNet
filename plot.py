...
df=pd.read_csv('/media/zlab-1/Data/Lian/keras/peak_data_new.csv',index_col=0)
...
plotbar([n['mean'][i+'_highgt'] for i in t],[n['mean'][i+'_highpd'] for i in t] , [n['std'][i+'_highgt'] for i in t], [n['std'][i+'_highpd'] for i in t], "End Diastolic Diameter(um)")
plotbar([n['mean'][i+'_lowgt'] for i in t],[n['mean'][i+'_lowpd'] for i in t] , [n['std'][i+'_lowgt'] for i in t], [n['std'][i+'_lowpd'] for i in t], "End Systolic Diameter(um)")
plotbar([60 * n['mean'][i+'_rategt'] for i in t],[60 * n['mean'][i+'_ratepd'] for i in t] , [60 * n['std'][i+'_rategt'] for i in t], [60 * n['std'][i+'_ratepd'] for i in t], "Heart Rate(BPM)")

...
direct = "/media/zlab-1/Data/Lian/keras/EP"
...
folder=folders[0:12]+folders[14:]
...
for i in folder:
    ...
    for j in file:
        ...
        #resized = img
        rotated = transform.rotate(resized, 90)

        if name.replace('.png','').isdigit() == True:
            train.append(resized)
            train.append(rotated)
...
model_checkpoint = ModelCheckpoint(direct+'/weights1.h5', monitor='val_loss', save_best_only=True)

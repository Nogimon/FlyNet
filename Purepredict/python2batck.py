...
def calculateArea(image, horirange, vertirange):
    ...
    for i in range(len(image)):
        ...
        vertdiameter = vertdiameter * float(yfactor)
        horidiameter = horidiameter * float(xfactor)
...
def runSeg(filepath, x, y):
    ...
    directorys = glob(filepath + "/*/*.tiff")
    for directory in directorys:
        print(directory, x, y)
        #if (directory == "*seg.tiff"):
            #continue
        ...
        #Post process
        countarea, countdiameter = calculateArea(a, HORIRANGE, VERTIRANGE)
        plotResult(directory, countarea, countdiameter)
        ...
        f = open(filepath + "FlyHeartDat.txt", "a+")
        s = directory + ", " + str(x) + "," + str(y) + "," + str(heartrate) + ", " + str(high_ave) + ", " + str(low_ave) + "\n"
        f.write(s)
        f.close()


if __name__ == "__main__":
    path = "/run/user/1000/gvfs/smb-share:server=128.180.65.184,share=home/Zlab-NAS3/Kate/262018/Larva/HCM2-/"
    runSeg(path, 24, 224)
    '''
    files = glob(path + "/*/*.tiff")
    for file in files:
        #filelocation, x position, y position
        runSeg(file, 24, 224)
    '''

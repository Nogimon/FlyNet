...
def get_range():
    abcd=[]
    for ii in ['EP','lava']:
        #folders=glob("/media/zlab-1/Data/QX/{}/*/".format(ii))
        folders=["/run/user/1000/gvfs/smb-share:server=128.180.65.173,share=data/Lian/flyheart/newdata/"]                                           # EP and lava
        for i in folders:
            print(i)
            if not glob(i+'*.tif'):
                continue
...
def save(b,d):
    for ii in ['EP','lava']:
        ...
        for i in folders:                                                              #crop and save
            ...
            label=glob(i+'*.tif')
            file=glob(i+'*.tiff')
            os.makedirs("/media/zlab-1/Data/QX/katefly/{}".format(os.path.basename(file[0])))
            ...
            for l in range(j,j+500):
                ...
                im1.save("/media/zlab-1/Data/QX/katefly/{}/{}_mask.png".format(os.path.basename(file[0]),l))
                img1.save("/media/zlab-1/Data/QX/katefly/{}/{}.png".format(os.path.basename(file[0]),l))
